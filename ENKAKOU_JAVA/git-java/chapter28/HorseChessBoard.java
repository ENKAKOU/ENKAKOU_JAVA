...
public class HorseChessBoard {
    ...
    public static void main(String[] args) {
        ...
        for (int[] rows : chessBoard) {     //输出当前这个棋盘的情况
    ...
    //马儿走的每一步step，记录到 chessBoard
    public static void traversalChessBoard(int[][] chessBoard, int row, int col, int step) {

        chessBoard[row][col] = step;     //先把step 记录到 chessBoard
        visited[row * X + col] = true;   //把这个位置，设置为已经访问

        ArrayList<Point> ps = next(new Point(col, row));  //获取当前这个位置可以走的下一个位置有哪?  col - X , row - Y
        sort(ps);   //排序.

        while (!ps.isEmpty()) {

            Point p = ps.remove(0);     //取出当前这个ps 第一个位置(点)
            if (!visited[p.y * X + p.x]) {    //判断该位置是否走过，如果没有走过，我们就递归遍历
                traversalChessBoard(chessBoard, p.y, p.x, step + 1);   //递归遍历
            }
        }

        if (step < X * Y && !finished) {   //当退出while，看看是否遍历成功, 如果没有成功，就重置相应的值，然后进行回溯
